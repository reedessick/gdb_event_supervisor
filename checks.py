...
def local_rates( gdb, gdb_id, verbose=False, window=5.0, rate_thr=5.0, event_type=None, timestamp="event_time" ):
    ...
    if timestamp=="event_time":
        ...
        event_time = float(sp.Popen("lalapps_tconvert %s"%gdb_entry['created'], stdout=sp.PIPE).communicate()[0])
        if verbose:
            print "\t\tcreated : %s -> %.6f"%(gdb_entry['created'], event_time)
        ### query for neighbors in (t-window, t+window), excluding this event
        winstart = sp.Popen("lalapps_tconvert %d"%np.floor(event_time-window), stdout=sp.PIPE).communicate()[0]
        winstop = sp.Popen("lalapps_tconvert %d"%np.ceil(event_time+window), stdout=sp.PIPE).communicate()[0]
        if verbose:
